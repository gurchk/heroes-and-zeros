...
function fetchBetsFromDB() {
  ...
  db.ref("bets/").on("child_added", snapshot => {
    ...
    if (!data.active) {
      // Om bettet inte är aktivt gör ngt
      // flytta det till en annan "tab" soom visar utgågna bet
    } else {
...
db.ref("bets/").on("child_changed", snapshot => {
  ...
  if (
    Object.keys(data.options).length === data.numberOfOptions &&
    data.active === true
  ) {
...
let calcTimeLeft = function(lastBetTime) {
  ...
  let timeLeft =
    new Date(lastBetTime).getTime() / 1000 - new Date(date).getTime() / 1000;
  if (timeLeft < 259200) {
    if (timeLeft < 172800) {
      if (timeLeft < 86400) {
        if (timeLeft < 0) {
...
let setTimeNow = function() {
  let timeNow;
  db
    .ref(`users/${user.uid}/currentTime`)
...
function filterByQuery(query) {
  ...
  for (let bet in bets) {
    ...
    if (
      ...
      obj.question != query &&
      obj.id != query &&
      obj.creator.uid != query &&
      obj.creator.name != query
    ) {
